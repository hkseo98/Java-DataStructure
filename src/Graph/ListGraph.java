...
public class ListGraph {
    ...
    public ArrayList getNode(int i) {
        return (ArrayList) this.listGraph.get(i);
    }
 
    // 그래프 추가 (양방향)
    public void put(int x, int y) {
        ((ArrayList) listGraph.get(x)).add(y);
        ((ArrayList) listGraph.get(y)).add(x);
    }
 ...
 // 그래프 추가 (단방향)
    ...
    public void printGraphToAdjList() {
        for(int i=1; i<listGraph.size(); i++) {
            ...
            for(int j=0; j<((ArrayList) listGraph.get(i)).size(); j++) {
                System.out.print(" -> " + ((ArrayList) listGraph.get(i)).get(j));
            }
            System.out.println();
        }
    }
